...
public class ViewFlow extends AdapterView<Adapter> {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(getDefaultSize(getSuggestedMinimumWidth(), widthMeasureSpec),largestHeight);
	}
	...
	private void postViewSwitched(int direction) {
		...
		if (direction > 0) { // to the right
			...
			if (newBufferIndex < mAdapter.getCount() && newBufferIndex > 0)
		...
		LogUtil.logOnlyDebuggable("ViewFlow", "mLoadedViews.size = "+mLoadedViews.size()+";mCurrentBufferIndex = " + mCurrentBufferIndex);
		
		if(mCurrentBufferIndex < 0 || mCurrentBufferIndex > mLoadedViews.size() -1)
			return;
