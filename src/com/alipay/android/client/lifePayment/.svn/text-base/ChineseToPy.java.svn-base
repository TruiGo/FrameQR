...
public class ChineseToPy
{
    ...
    //单个汉字转换成全拼
	public static String getFullPy(String st)
	{

		String py ="";
		String hz ="";
        for (int i = 0 ;i <g_hz.length;i++){
           py =  g_hz[i][0];
           hz =  g_hz[i][1];
           if (hz.indexOf(st) != -1){
			     py = py.toLowerCase();
	 ...
	 //单个汉字转换成拼音第一个字母
	public static String getSinglePy(String st)
	{
        ...
        if("".equals(py) && st.matches("[a-zA-Z]+.*")){
	...
	//字符串转换成全拼音
	public String getStringFullPY(String s){
		String ret = "";
		byte[] b =  s.getBytes();
		for (int i = 0 ;i<b.length;i++){
			if (b[i]>=0) //如果该字符不是汉字，则忽略
				continue;
		 	byte[] hz = new byte[2];
		 	hz[0] = b[i];
		 	hz[1] = b[i+1];
		 	try{
		 		...
		 		String strHz = new String(hz,"gb2312");
		 		String py =getFullPy(strHz);
		 		ret += py;
		 	}catch(Exception ee){}
			i++;
	...
	//字符串转换拼音第一个字母
	public String getStringFirstPY(String s){
		String ret = "";
		byte[] b =  s.getBytes();
		for (int i = 0 ;i<b.length;i++){
			if (b[i]>=0) //如果该字符不是汉字，则忽略
