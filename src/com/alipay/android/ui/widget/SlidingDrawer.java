...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        mAnimateOnClick = a.getBoolean(R.styleable.SlidingDrawer_animateOnClick, false);
    ...
    private boolean isLowLevel(){
    	return (Utilz.getCPUFrequencyMax() > 0 && Utilz.getCPUFrequencyMax() < 1000);
    }
                    ...
                    if (Math.abs(velocity) < mMaximumTapVelocity) {
                        if (vertical ? (mExpanded && top < mTapThreshold + mTopOffset) ||
                            ...
                            if (mAllowSingleTap) {
                                ...
                                if (mExpanded) {
                                	if(frame.contains((int) x, (int) y) || mAnimating){
                                		prepareTracking(COLLAPSED_FULL_CLOSED);
                                		stopTracking();
                                		closeDrawer();
                                	}else{
                                	...
                                	if(frame.contains((int) x, (int) y) || mAnimating){
                                		animateOpen(EXPANDED_FULL_OPEN);
                                	}else{
...
//        if (Build.VERSION.SDK_INT > 11 && !content.isHardwareAccelerated()) content.buildDrawingCache();
    ...
    private void incrementAnimation() {
        ...
        mAnimatedVelocity = v * 4f + (a * t);                           // px/s
