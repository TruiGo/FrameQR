...
public class PluginManager {
     ...
     * @param ctx
     */
    public PluginManager(WebView app, Page page) {
        mPage = page;
        this.app = app;
        this.firstRun = true;
    }
    ...
    public void loadPlugins() {
        ...
        try {
            ...
            xml.setInput(mPage.getRunTime().getAssets().open("plugins.xml"), "utf-8");
        } catch (Exception e) {
            pluginConfigurationMissing();
            e.printStackTrace();
        }//Resources().getIdentifier("plugins", "xml", ctx.getPackageName());
        int eventType = -1;
    ...
    public void startupPlugins() {
        for (PluginEntry entry : this.entries.values()) {
            if (entry.onload) {
                entry.createPlugin(this.app, mPage);
            }
        }
    }
     ...
     */
    public String exec(final String service, final String action, final String callbackId,
        ...
        try {
            ...
            if (plugin != null) {
                ...
                if (runAsync) {
                        ...
                        public void run() {
                            try {
                                ...
                                // Check the success (OK, NO_RESULT & !KEEP_CALLBACK)
                                    ...
                                    page.sendJavascript(cr.toSuccessCallbackString(callbackId));
                                }

                                // If error
                                else {
                                    page.sendJavascript(cr.toErrorCallbackString(callbackId));
                                }
                            } catch (Exception e) {
                    ...
                    // If no result to be sent and keeping callback, then no need to sent back to JavaScript
                    if ((cr.getStatus() == PluginResult.Status.NO_RESULT.ordinal())
                        && cr.getKeepCallback()) {
                        return "";
                    }
                }
        ...
        if (runAsync) {
            ...
            mPage.sendJavascript(cr.toErrorCallbackString(callbackId));
        }
    ...
    private IPlugin getPlugin(String service) {
        ...
        if (plugin == null) {
            plugin = entry.createPlugin(this.app, mPage);
        }
        return plugin;
    }
    ...
    private void pluginConfigurationMissing() {
        System.err
            .println("=====================================================================================");
        System.err
            .println("ERROR: plugin.xml is missing.  Add res/xml/plugins.xml to your project.");
        System.err
            .println("https://git-wip-us.apache.org/repos/asf?p=incubator-cordova-android.git;a=blob;f=framework/res/xml/plugins.xml");
        System.err
            .println("=====================================================================================");
    }
}
